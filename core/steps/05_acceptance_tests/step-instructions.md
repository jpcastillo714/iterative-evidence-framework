...
El objetivo principal de este paso es traducir cada regla definida en el paso anterior en al menos una prueba de aceptación ejecutable. Estas pruebas deben utilizar el formato estandarizado *Given/When/Then* (Dado/Cuando/Entonces) para asegurar que el comportamiento del sistema esté alineado de manera precisa con los requisitos declarados, proporcionando criterios de éxito claros, medibles y verificables.

## Contexto

**Prerrequisito:** El Paso 4 (las reglas) debe estar COMPLETADO. Las reglas aprobadas son la materia prima de este paso.
...
5. **Trazabilidad estricta:** Cada test DEBE referenciar una regla por ID (ej. linked_rule: RUL-001-001). Tests sin trazabilidad son inválidos.

1. **Cobertura Total (1:1 o más):** Cada regla (`RUL-NNN-XXX`) DEBE tener al menos una prueba de aceptación asociada (`TST-ACC-XXX`). No pueden existir reglas huérfanas sin pruebas.
...
5. **Trazabilidad Contractual:** Cada prueba debe referenciar de forma explícita qué regla del proyecto valida y cuáles campos del contrato de datos (Data Contract) están involucrados en la prueba.
...
2. **Mapeo y Generación de Pruebas:**
   Por cada regla del proyecto identificada (`RUL-NNN-XXX`), redacta al menos una prueba de aceptación (`TST-ACC-XXX`). Si una regla es compleja, divídela en múltiples pruebas para abordar diferentes escenarios.
...
4. **Cobertura de Casos Especiales (Edge Cases):**
   Para aquellas reglas clasificadas con prioridad CRITICAL o HIGH, debes diseñar y añadir pruebas que aborden casos límite (edge cases) y condiciones de falla.
...
- Existe un 100% de cobertura de las reglas (todas las reglas tienen al menos una prueba).
...
- **Reglas Olvidadas:** Dejar reglas sin pruebas asociadas.
